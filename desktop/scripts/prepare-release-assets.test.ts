...
test('merges macOS architectures and emits checksums for a complete release', async () => {
  ...
  await asset(input, 'win', 'WizeStream Desktop-0.6.0-beta-windows-x64.exe');
  await metadata(input, 'win', 'beta.yml', 'WizeStream Desktop-0.6.0-beta-windows-x64.exe');
  for (const arch of ['x64', 'arm64']) {
    await asset(input, `mac-${arch}`, `WizeStream Desktop-0.6.0-beta-macos-${arch}.dmg`);
    await asset(input, `mac-${arch}`, `WizeStream Desktop-0.6.0-beta-macos-${arch}.zip`);
    await metadata(input, `mac-${arch}`, 'beta-mac.yml', `WizeStream Desktop-0.6.0-beta-macos-${arch}.zip`);
    await asset(input, `linux-${arch}`, `WizeStream Desktop-0.6.0-beta-linux-${arch}.AppImage`);
    await metadata(input, `linux-${arch}`, arch === 'x64' ? 'beta-linux.yml' : 'beta-linux-arm64.yml',
      `WizeStream Desktop-0.6.0-beta-linux-${arch}.AppImage`);
  }
  ...
  expect(mac.files.map((file) => file.url)).toEqual([
    'WizeStream Desktop-0.6.0-beta-macos-arm64.zip',
    'WizeStream Desktop-0.6.0-beta-macos-x64.zip',
  ]);
...
async function metadata(root: string, directory: string, name: string, url: string): Promise<void> {
  await writeFile(path.join(root, directory, name), dump({
    version: '0.6.0-beta', files: [{ url, sha512: `sha512:${url}`, size: 42 }],
