    ...
    fun show(
        ...
        dialog.setOnShowListener {
            dialog.getButton(android.app.AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                val text = binding.learningNoteText.text?.toString()?.trim().orEmpty()
                ...
                binding.learningNoteTextLayout.error = when {
                    text.isEmpty() -> context.getString(R.string.learning_note_empty_error)

                    text.length > LearningNoteManager.MAX_NOTE_LENGTH -> context.getString(
                ...
                if (timestamp != null && text.isNotEmpty() &&
                    text.length <= LearningNoteManager.MAX_NOTE_LENGTH
                ) {
                    onSave.onSave(timestamp, text)
                    dialog.dismiss()
                }
            }
        }
